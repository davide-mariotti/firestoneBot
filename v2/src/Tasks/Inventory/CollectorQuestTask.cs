...
/// <summary>
///     Progresses the daily quest "Collector" (open 4 gear chests) by opening every gear chest in
///     the bag except a reserve of common chests (kept so the quest is never blocked the next day
///     too - see min_common_reserve). No v1 precedent - chest opening was never automated before.
...
///     non-gear-chest slots (jewel/celestial chests, mystery box, etc). Flag for live verification:
///     if a rarity slot has a different real name than assumed, it's still reachable through this
///     generic scan since nothing here depends on knowing the name in advance.
/// </summary>
public class CollectorQuestTask : BotTask
{
    ...
    public override IEnumerator Execute()
    {
        ...
        var nonGearChestSlots = new HashSet<string>(Paths.InventoryLoc.KnownNonGearChestSlots);
