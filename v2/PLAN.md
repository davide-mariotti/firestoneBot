...
| 12 | **Firestone Research** | Esiste | Fatto. Porting diretto della logica di selezione talenti (scansiona tutti i 16 nodi ad ogni slot libero, sceglie sempre il livello più basso, a parità il tempo minore) + navigazione manuale esplicita (rightSideUI/townButton → TownIrongard/townBg/parent/library → submenuButtons/firestoneResearch), non presente in v1. Nota: la schermata Library ha 2 tab (meteoriteResearch/firestoneResearch) e quello giusto NON è selezionato di default all'apertura (docs/screens/Library.html) — mancava nel primo giro, trovato da un audit e corretto (click sul tab aggiunto). Path del tab non verificato dal vivo (nessun corrispettivo v1, solo fonte doc). |
